...
(function() {
  ...
  module.exports = ServerApp = (function(_super) {
    ...
    ServerApp.prototype.initMongo = function() {
      ...
      return this.startEventStream();
    };

    ServerApp.prototype.startEventStream = function() {
      var app;
      app = this;
      this.eventStream = this.EventModel.find().tailable().limit(1).stream();
      this.eventStream.on('error', function(err) {
        return console.error(err);
      });
      return this.eventStream.on('data', function(event) {
        if (event.action === 'game.join') {
          return app.joinGame(event);
        }
      });
    };

    ServerApp.prototype.joinGame = function(event) {
      var app;
      app = this;
      return this.GameModel.findById(event.game).populate('players').exec(function(err, game) {
        var gameSockets;
          ...
          gameSetup = function(game) {
            ...
            return player.save(function(err) {
              ...
              return game.save(function(err) {
                ...
                event.save();
                return socket.emit('playerSetup.complete', player);
              });
            });
          };
          if (gameId) {
